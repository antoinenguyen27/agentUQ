---
title: Get Started
description: The fastest path to installing AgentUQ, running the minimal loop, and understanding what to read next.
slug: /get-started
sidebar_position: 2
---

# Get Started

Use this section when you want the shortest path from clone to informed runtime use.

## Recommended order

1. [Install AgentUQ](install.md).
2. [Run the minimal quickstart loop](quickstart.md).
3. Read [Acting on decisions](../concepts/acting_on_decisions.md) if you need runtime routing patterns.
4. Read [Reading results](../concepts/reading_results.md) once you start looking at real output.
5. Jump to [Quickstarts](../quickstarts/index.md) for your provider or framework surface.

## What this section covers

- package installation and local setup
- the `capture -> analyze -> decide` loop
- decision routing at the step level
- readable terminal output and next-step interpretation

## What this section does not try to do

- It does not replace the provider-specific quickstarts.
- It does not replace the deeper statistical framing in [Research grounding](../concepts/research_grounding.md).
- It does not repeat the full object-by-object API inventory from the [API reference](../concepts/public_api.md).

